                  ...
                  <defs>
                    ...
                    <clipPath id="clip0_18078_104881">
                      <rect width="34" height="34" fill="white" />
                        ...
                        <!--<h2 class="text-base-content text-xl font-bold"></h2>-->
                    </a>
                </div>
                <div>
                    <h3 class="text-base-content mb-1.5 text-2xl font-semibold">{{ __('Sign Up') }}</h3>
                    <p class="text-base-content/80">{{ __('Focusing on growth doesn\'t mean ignoring results.') }}</p>
                </div>
                <div class="space-y-4">
                    <form class="mb-4 space-y-4" action="{{ route('register') }}" method="POST">
                        @csrf
                        <div>
                            <label class="label-text" for="userName">{{ __('Username') }}*</label>
                            <input type="text" name="name" placeholder="{{ __('Enter your username') }}" class="input @error('name') input-error @enderror" id="userName" value="{{ old('name') }}" required />
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="email" name="email" placeholder="{{ __('Enter your email address') }}" class="input @error('email') input-error @enderror" id="userEmail" value="{{ old('email') }}" required />
                            @error('email')
                            <span class="text-error text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <label class="label-text text-base-content/80 p-0 text-base" for="policyagreement">{{ __('I agree to ') }}<a href="#" class="link link-animated link-primary font-normal">{{ __('privacy policy & terms?') }}</a> </label>
                        </div>
                        <button class="btn btn-lg btn-primary btn-gradient btn-block" type="submit">{{ __('Sign Up') }}</button>
                    </form>
                    <p class="text-base-content/80 mb-4 text-center">
                        {{ __('Already have an account?') }}
                        <a href="{{ route('login') }}" class="link link-animated link-primary font-normal">{{ __('Sign in instead') }}</a>
                    </p>
                </div>
            </div>
        </div>
